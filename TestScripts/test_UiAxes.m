...
function GUI(x, qoi, lbl, plotdes, extraopt, designEvaluator)
    ...
    for i = 1:nVars
        ...
        % Label for Lower and Upper bound
        ...
        uicontrol('Parent', variablePanel, 'Style', 'text', 'String', 'Upper Bound', ...
                  'Units', 'normalized', 'Position', [0.80, 0.7, 0.2, 0.3]);

        % Slider for lower bound (doubled thickness)
        sliderHandles(i) = uislider( variablePanel, ...
                                        "range", ...               % Correct 'Style' property for a range slider
                                        'Limits', [x(i).dslb x(i).dsub], ...  % Min and Max range
                                        'Value', [x(i).sblb x(i).sbub], ...   % Initial range values
                                        'Step', 0.1, ...
    ...
    uicontrol('Parent', hFig, 'Style', 'pushbutton', 'String', 'Update Plot', ...
              ...
              'Callback', @(src, event) updatePlot(hFig,lbl,plotdes,extraopt,designEvaluator,nVars));
    ...
    h = SolutionSpace_plot(hFig,lbl,plotdes,extraopt,designEvaluator, 'Figure', figureBox);
...
function updatePlot(hFig,lbl,plotdes,extraopt,designEvaluator,nVars)
    ...
    try
        ...
        h = SolutionSpace_plot(hFig,lbl,plotdes,extraopt,designEvaluator);
...
function h = SolutionSpace_plot(hFig, lbl,plotdes,extraopt,designEvaluator,varargin)
    global PlotData OptimdesignBox x;

    parser = inputParser;
    parser.addParameter('Figure', [], @(x) isempty(x) || ishandle(x)); % Allow optional panel
    parser.parse(varargin{:});
	options = parser.Results;

    %% Computing Selective Design Space Projection
    ...
    for i = 1:size(plotdes, 2)
        desiredPairs(i, :) = [plotdes(i).axdes{1}, plotdes(i).axdes{2}];  % Fill in the rows
        axisLabel{i} = lbl(i).dispname;  % Store each name in the cell array 
    end

    [PlotData,~] = evaluate_selective_design_space_projection(...
        designEvaluator,...
        designBox,...
        [x.dslb],...
        [x.dsub],...
    ...
    % Get parent panel
    parentPanel = options.Figure;

    % Number of plots (determined by input data)
    nPlots = numel(plotdes); % Assuming plotdes determines the number of plots
    ...
    nCols = min(nPlots, maxCols); % Max 3 columns
    nRows = min(ceil(nPlots / maxCols), maxRows); % Max 3 rows

    % Spacing parameters (adjust as needed)
    marginX = 0.075;   % Horizontal margin
    marginY = 0.1;     % Vertical margin
    spacingX = 0.0875; % Space between axes (horizontal)
    spacingY = 0.0875; % Space between axes (vertical)

    % Compute width and height of each axis dynamically
    axWidth = (1 - (marginX * 2) - (spacingX * (nCols - 1))) / nCols;
    axHeight = (1 - (marginY * 2) - (spacingY * (nRows - 1))) / nRows;

    % Store axes handles
    hAxes = gobjects(1, nPlots);

    % desired plots by design variable index
    plotGrid = [1,size(plotdes, 2)]; % 2 rows, 3 columns of subplots

    % Create axes dynamically
    for i = 1:size(desiredPairs,1)
        row = ceil(i / maxCols); % Compute row index
        col = mod(i-1, maxCols) + 1; % Compute column index
        
        % Compute position for each axis
        posX = marginX + (col - 1) * (axWidth + spacingX);
        posY = 1 - (marginY + row * axHeight + (row - 1) * spacingY); % Invert Y for MATLAB UI
        
        % Create axes
        hAxes(i) = axes('Parent', parentPanel, ...
            'Units', 'normalized', ...
            'Position', [posX, posY, axWidth, axHeight], ...
            'Tag', sprintf('Plot%d', i), ...
            'XLimMode', 'manual', ...
            'YLimMode', 'manual', ...
            'NextPlot', 'replacechildren'); % Prevent persistent overlays
        
        % Fix axis limits
        xlim(hAxes(i), xlim(hAxes(i)));
        ylim(hAxes(i), ylim(hAxes(i)));
        
        % Disable zoom, pan, and rotation, but NOT interaction with objects
        zoom(hAxes(i), 'off');
        pan(hAxes(i), 'off');
        rotate3d(hAxes(i), 'off');
        
        % Make sure axes remain fixed but draggable lines are still interactive
        axis(hAxes(i), 'manual');
        
        % Prevent MATLAB from overriding settings
        set(hAxes(i), 'ClippingStyle', 'rectangle'); % Prevent zoom resetting axes
        
        % Allow object interactions (important for draggable lines)
        set(hAxes(i), 'HitTest', 'on');  % Allow mouse clicks on axes
        set(hAxes(i), 'PickableParts', 'all'); % Ensure objects inside axes can be clicked

        % Define Design variable pairs
        currentPair = [desiredPairs(i,1),desiredPairs(i,2)];
        currentPairLabels = axisLabel(currentPair);

        designTypeHandle(i) = plot_design_space_projection_axis(PlotData(i),...
            'CurrentAxis', hAxes(i), ...
    ...
    h.IndividualPlots= hAxes;
    h.GoodPerformance = designTypeHandle.GoodPerformance
    h.BadPerformance = designTypeHandle.BadPerformance
    ...
    for i = 1:length(h.IndividualPlots)
        ...
        xVarIndex = desiredPairs(i, 1); % X variable index
        yVarIndex = desiredPairs(i, 2); % Y variable index
