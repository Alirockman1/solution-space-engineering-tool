...
% Global Variables
global textHandles x sliderHandles h PlotData OptimdesignBox;
...
% designProblemsFolder = fullfile(projectRoot, 'ProblemDefinition');
...
if isempty(mFiles)
    ...
    for n_mfiles = 1:length(mFiles)
        % Remove the .m extension and display the numbered function name
        [~, functionName, ~] = fileparts(mFiles(n_mfiles).name);
        fprintf('%d. %s\n', n_mfiles, functionName);
    end
...
function GUI(x, qoi, lbl, plotdes, extraopt, designEvaluator)
    ...
    % Define Variables
    global textHandles sliderHandles h OptimdesignBox;

    %% Create GUI layout
    % Get the screen size for fullscreen
    screenSize = get(0, 'ScreenSize'); 
    screenWidth = screenSize(3);  % Screen width
    screenHeight = screenSize(4); % Screen height

    % Create the figure in fullscreen mode
    hFig = figure('Name', 'Solution Space GUI', 'NumberTitle', 'off', ...
    ...
    mainPanel = uipanel('Parent', hFig, 'Title', 'Design Variables', ...
                        'Position', [0.05 0.6 0.35 0.35]);  % Adjust the panel size

    % Sliders and textboxes for each design variable
    nVars = length(x);  % Number of design variables
    sliderHandles = gobjects(nVars, 1);  % Store slider handles
    textHandles = gobjects(nVars, 2);    % Store text box handles for lower and upper bounds
    ...
    for i = 1:nVars
        ...
        % Label for the bounds
        uicontrol('Parent', variablePanel, 'Style', 'text', 'String', '', ...
                  'Units', 'normalized', 'Position', [0.01, 0.5, 0.2, 0.3]);

        % Two-way slider (superslider) for both bounds
        sliderHandles(i) = superSlider(variablePanel, 'Position', [0.15, 0.30, 0.70, 0.50],...
                                       'Min', (x(i).dslb), 'Max', (x(i).dsub), ...
                                       'Value', [x(i).sblb x(i).sbub], 'NumSlides', 2, ...
                                       'Units', 'normalized',...
                                       'tagName',['slider' i],...
                                       'stepSize', 1,...
                                       'Callback', @(src, event) slider_callback(i));
        ...
        textHandles(i, 1) = uicontrol('Parent', variablePanel, 'Style', 'edit', 'String', num2str(x(i).sblb, '%.2f'), ...
                                      ...
                                      'Callback', @(src, event) textbox_callback(i, 1, variablePanel)); % Lower Text Handle
        ...
        textHandles(i, 2) = uicontrol('Parent', variablePanel, 'Style', 'edit', 'String', num2str(x(i).sbub, '%.2f'), ...
                                      ...
                                      'Callback', @(src, event) textbox_callback(i, 2, variablePanel)); % Upper Text Handle
    end
    ...
    savePathEdit = uicontrol('Parent', postProcessingPanel, 'Style', 'edit', ...
        'Units', 'normalized', 'Position', [0.35, 0.4, 0.45, 0.1], 'String', defaultSavePath);
    
    % Button for saving figure and results
    uicontrol('Parent', postProcessingPanel, 'Style', 'pushbutton', 'String', 'Save Results', ...
        'Units', 'normalized', 'Position', [0.15, 0.1, 0.7, 0.2], 'Callback', ...
        @(src, event) saveResultsCallback(excelNameEdit, figureNameEdit, savePathEdit, lbl, qoi));

    %% Create the figure box for placing figure
    figureBox = uipanel('Parent', hFig, 'Title', 'Solution Space', ...
        'Position', [0.45, 0.465, 0.55, 0.45]);
    
    axes('Parent', hFig, 'Position', [0.20, 0.55, 0.9, 0.6]);
    axis off;  % Turn off axis for the figure box

    %% Add buttons for Optimization and Rerun Plot
    % Update Plot button at the bottom of the GUI
    uicontrol('Parent', hFig, 'Style', 'pushbutton', 'String', 'Update Plot', ...
              'Units', 'normalized', 'Position', [0.60, 0.915, 0.2, 0.08], ...
    ...
    uicontrol('Parent', hFig, 'Style', 'pushbutton', 'String', 'Optimize', ...
              'Units', 'normalized', 'Position', [0.825, 0.915, 0.1, 0.08], ...
...
function slider_callback(varIndex)
    ...
    global h sliderHandles textHandles;
    disp(['Slider ', num2str(varIndex), ' moved.']);

    if ishandle(sliderHandles(varIndex))
        infoMatrix = get(sliderHandles(varIndex), 'UserData');
        newvalue_lower = round(infoMatrix(1,1),2);
        newvalue_upper = round(infoMatrix(1,2),2);

        % Update text boxes with current slider values
        set(textHandles(varIndex, 1), 'String', num2str(newvalue_lower));

        % Update text boxes with current slider values
        set(textHandles(varIndex, 2), 'String', num2str(newvalue_upper));
    ...
    drag_lines(h,desiredPlots,x)
end

%% Function to update sliders based on text input
function textbox_callback(varIndex, boundType, variablePanel)
    % Function to retrieve data from the slider.
    ...
    slider = findobj('tag',['slider' varIndex]);
    current_slider_values = get(slider,'UserData')
    slider_position = slider.Position;
    delete(slider);

    % Check if the entered Value is within bounds
    if (boundType == 1 && newValue >= x(varIndex).dslb)
        
        new_slider_values = [newValue current_slider_values(1,2); 1 0]

        sliderHandles(varIndex) = superSlider(variablePanel,...
            'Min', (x(varIndex).dslb), ...
            'Max', (x(varIndex).dsub), ...
            'numSlides',2,...
            'Units', 'normalized',...
            ...
            'stepSize',1,...
            'tagName',['slider' varIndex],...
            @(src, event) slider_callback(i));
        sliderHandles(varIndex).Position = slider_position;

    elseif boundType == 2 && newValue <= x(varIndex).dsub
        set(sliderHandles(varIndex), 'Value', [other_slider(1,1) newValue]);
    end
...
%% Function to Plot Draggable boundaries
function drag_lines(h,desiredPlots,x)
    for i = 1:length(h.IndividualPlots)
