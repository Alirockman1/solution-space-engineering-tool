...
% The function performs DSA of displacements using SA DDM - Semi-Analytical
% Direct Differentiation Method

function [O] = DispDSA_DDM(G, A, O)
...
% Calculate displacement sensitivities using SA DDM
% Loop over all design variables
%for DVNo = 1:O.nDesVar
  DVNo = 1; % We only perform DSA w.r.t. the first design variable
  % Reset the pseudo load vector P for every design variable
  P = zeros(A.nDOFs,1);
  ...
  for No = 1:O.DVInfo(DVNo).nPatchToElemLinks
    ...
    % Compute the contribution to the pseudo load vector
    P_e = -DKeDx*E.U(:,1);
    P(E.DOF) = P(E.DOF) + P_e;
  end % loop for No
  % Solve the sensitivity equation for displacement sensitivities using 
  % the factored stiffness matrix
  O.DUDx_DDM(A.FreeDOFs,:) = A.KFactored \ P(A.FreeDOFs,:);
  O.DUDx_DDM(A.FixedDOFs,:)= 0.0;
  % The displacement sensitivity value of interest
  O.DUzDx_DDM(DVNo) = O.DUDx_DDM(DOFNo,1);
