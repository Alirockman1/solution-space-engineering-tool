...
% This variant of AdaptiveMoveLimit uses individual move limits

function [xLower, xUpper] = AdaptiveMoveLimitVar(O, x, k, xL, xU, ...
...
% The move limits defined as O.ML(DVNo) are used
  ...
  if (k > 1)
    ...
    for i =1:O.nDesVar
      delta = (xU(i)-xL(i))*O.ML(i)/2;   % Start out slowly by using MoveLimit/2
      % Initial extimate of lower and upper bound on x(i)
      xLower(i) = x(i) - delta;
      xUpper(i) = x(i) + delta;
      % Make sure we are within the feasible domain
