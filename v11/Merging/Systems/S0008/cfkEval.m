...
G.LX = 0.05;       % Length of plate (x) is 0.5 m
G.LY = 0.1;       % Height of plate (y) is 0.1 m
G.nElemX = 5;    % Number of elements in x-direction
G.nElemY =  10;    % Number of elements in y-direction
...
G.SymLayup = 1;   % Symmetric layup is not enforced
G.F = 100;
...
G = DefineLayup(G, length(alpha), ...
    .7e-4*ones(1,length(alpha)), ...
    3*ones(1,length(alpha)), ...
    alpha);
% G = DefineLayup(G, 4, [0.0001 0.0001 0.0001 0.0001], [3 3 3 3], [90 45 -45 0]); % 4-layer quasi-isotropic CFRP
...
% t = sum(0.0001*ones(1,length(alpha)));

% Eby = G.StiffMat(5,5) * 12 / (t^3);
% Iby = G.LY * t^3 / 12;
f = G.F * G.LX^3 / (3*G.StiffMat(4,4) * G.LY );
% end
...
% PlotDisp(G, A, 1);
% PlotDisp(G, A, 2);
A.Disp = PlotDisp(G, A, 3); % PLOTTING DISABLED

A.maxDisp = max(A.Disp, [], 'all');
% PlotDisp(G, A, 4);
% PlotDisp(G, A, 5);
...
% PlotElemStr(G, A, 'StressXY', 1, 2, 1);
% PlotElemStr(G, A, 'StressXY', 2, 2, 1);
% PlotElemStr(G, A, 'StressXY', 3, 2, 1);
