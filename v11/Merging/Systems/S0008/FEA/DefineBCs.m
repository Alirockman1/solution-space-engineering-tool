...
switch DispBC
 ...
 case 6
        % Clamped plate. All edges EXCEPT RIGHT ONE are completely clamped
	A.nFixedDOFs = 1*5*(G.nElemY*2+1) + 2*5*(G.nElemX*2-1);
	A.FixedDOFs = zeros(A.nFixedDOFs,1);
    % Left edge
	i = 0;
    for NodeNo = 1:G.nElemY*2+1
...
case 7
    % Clamped plate. All edges EXCEPT UPPER ONE are completely clamped
	A.nFixedDOFs = 2*5*(G.nElemY*2+1) + 2*5*(G.nElemX*2-1);
	A.FixedDOFs = zeros(A.nFixedDOFs,1);
    % Left edge
	i = 0;
