%% S0005_x_CrashDesignProblem.m

% Template generated automatically
% 
classdef S0005_x_CrashDesignProblem < handle
    properties
		%--------------------------------------------------
		...
		save_as = 'S0005_x_CrashDesignProblem.mat';
	end

    methods
		function obj = S0005_x_CrashDesignProblem()
			addpath('Data\Save');
			try
				load(obj.save_as);
				obj = OBJ;
       ...
       % Calculates system response
		function [y] = SystemResponse(obj, x)
			...
			% Function :
			y = S0005_f_CrashDesignProblem(x);
