...
switch LoadBC
  ...
  case 7
    ...
    case 10
    % Point load FZ at the top of the right edge
    MidNodeNo = G.nElemX * (G.nElemY*4+2) + 1;
    DOFNo = 5*(MidNodeNo-1)+3;
    A.F(DOFNo) = A.F(DOFNo) + ScaleFac;

    case 11
    % Constant transverse load/length N_z in (x-) middle
    StartNo = G.nElemX * (G.nElemY*2+1) + 1;
    E.Nodes(1:3) = 0;
    NodeNo = StartNo;
    % Corner nodes get 1/6 while midnode get 2/3 of total load/length (ScaleFac*G.ElemLY)
    for ElemNo = 1:G.nElemY
      ...
      NodeNo = E.Nodes(2);
      DOFNo = 5*(NodeNo-1)+3;
      A.F(DOFNo) = A.F(DOFNo) + ScaleFac*2/(3*G.nElemY);
      % The corner nodes get 1/3
      for i = 1:2:3
        NodeNo = E.Nodes(i);
        DOFNo = 5*(NodeNo-1)+3;
        A.F(DOFNo) = A.F(DOFNo) + ScaleFac/(6*G.nElemY);
      end
    end
