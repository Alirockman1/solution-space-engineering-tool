%% S0006_x_DemonstrationCrashProblem.m
% Template generated automatically
% 
classdef S0006_x_DemonstrationCrashProblem < handle
    properties
		%--------------------------------------------------
		...
		save_as = 'S0006_x_DemonstrationCrashProblem.mat';
	end

    methods
		function obj = S0006_x_DemonstrationCrashProblem()
			addpath('Data\Save');
			try
				load(obj.save_as);
				obj = OBJ;
       ...
       % Calculates system response
		function [y] = SystemResponse(obj, x)
			...
			% Function :
			y = S0006_f_DemonstrationCrashProblem(x);
