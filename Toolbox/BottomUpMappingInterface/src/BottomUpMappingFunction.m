...
%   limitations under the License.
	...
	methods
        ...
        %   See also car_crash_2d, distance_to_center, two_ellipsoids.
			...
			elseif(nargout(obj.SystemFunction)==3)
				[performanceMeasure,physicalFeasibilityMeasure,systemFunctionOutput] = obj.SystemFunction(systemFunctionInputArgument{:});
			else % possibly using lamba function - try from the most specific to the least specific
				try
					[performanceMeasure,physicalFeasibilityMeasure,systemFunctionOutput] = obj.SystemFunction(systemFunctionInputArgument{:});
				catch
					try
            ...
            if(isempty(physicalFeasibilityMeasure))
				...
				elseif(nargout(obj.PhysicalFeasibilityFunction)==2)
					...
					try 
						[physicalFeasibilityMeasure,physicalFeasibilityFunctionOutput] = obj.PhysicalFeasibilityFunction(physicalFeasibilityFunctionInputArgument{:});
					catch
						[physicalFeasibilityMeasure] = obj.PhysicalFeasibilityFunction(physicalFeasibilityFunctionInputArgument{:});
					end
                end
			...
			% check if output has correct size
			if(size(performanceMeasure,1)~=size(designSample,1))
				error('BottomUpMappingFunction:InvalidPerformanceMeasureSize',...
					'Performance measure must have the same number of rows as design sample.');
			end
			if(~isempty(physicalFeasibilityMeasure) && size(physicalFeasibilityMeasure,1)~=size(designSample,1))
				error('BottomUpMappingFunction:InvalidPhysicalFeasibilityMeasureSize',...
					'Physical feasibility measure must have the same number of rows as design sample.');
			end
