...
%
%       - 'DoNotAdaptGrowthRateOnFullPurity' : logical, whether the growth 
%         rate should be adapted only if the purity<1. Default: true.
%
...
%
%       - 'MinimumPurityReset' : double, minimum purity level to reset the 
%         current iteration without continuing the process. With an adaptive 
%         growth rate, this means in the next iteration that growth rate will
%         be significantly reduced. Default: 0.15.
%
...
%   limitations under the License.
    ...
    parser.addParameter('UseAdaptiveGrowthRate',true,@(x)islogical(x)&&isscalar(x));
    parser.addParameter('DoNotAdaptGrowthRateOnFullPurity',true,@(x)islogical(x)&&isscalar(x));
    ...
    parser.addParameter('SaveIterationAllData',false,@(x)islogical(x)&&isscalar(x));
    ...
    if(strcmpi(solutionSpaceType,'component'))
        % sampling
        parser.addParameter('MinimumNumberPaddingSamples',[],@(x)isnumeric(x)&&all(x>=0));
        ...
        parser.addParameter('ShapeType',[],@(x)isempty(x) || any(strcmpi(x,{'advanced','simple'})));
        parser.addParameter('ShapeTypeExploration','advanced',@(x)isempty(x) || any(strcmpi(x,{'advanced','simple'})));
        parser.addParameter('ShapeTypeConsolidation','simple',@(x)isempty(x) || any(strcmpi(x,{'advanced','simple'})));
        % candidate spaces
        parser.addParameter('CandidateSpaceConstructor',@CandidateSpacePlanarTrimming,@(x)isa(x,'function_handle'));
    ...
    %% options exclusive to component SSO
    if(strcmpi(solutionSpaceType,'component'))
        if(~isempty(options.ShapeType))
            options.ShapeTypeExploration = options.ShapeType;
            options.ShapeTypeConsolidation = options.ShapeType;
        end
    end
end
