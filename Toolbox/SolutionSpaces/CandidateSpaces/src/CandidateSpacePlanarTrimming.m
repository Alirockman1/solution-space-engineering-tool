...
%   limitations under the License.
    ...
    properties (SetAccess = protected)
        ...
        %   GROWTHDIRECTIONTYPE is a string that determines the type of growth direction.
        %   It can be 'plane-direction' or 'away-from-center'.
        %
        ...
        %REDUNDANCYROUNDINGTOLERANCE Tolerance for rounding the plane direction
        %   REDUNDANCYROUNDINGTOLERANCE is the tolerance for rounding the plane 
        %   direction to avoid numerical issues.
        %
        %   REDUNDANCYROUNDINGTOLERANCE : double
        RedundancyDigitRoundingTolerance
    end
    ...
    methods
        ...
        %   See also convex_hull_plane.
            ...
            parser.addParameter('CheckRedundancyCadence',1,@isnumeric);
            parser.addParameter('PerformDetailedRedundancyCheck',true,@islogical);
            parser.addParameter('UseOnlyOtherAnchorsInRedundancyCheck',false,@islogical);
            parser.addParameter('MeasureEstimationFactor',10,@isnumeric);
            parser.addParameter('RedundancyDigitRoundingTolerance',10,@isnumeric);
            parser.addParameter('GrowthDirectionType','plane-direction',@(x)any(strcmpi(x,{'plane-direction','away-from-center'})));
            ...
            obj.RedundancyDigitRoundingTolerance = parser.Results.RedundancyDigitRoundingTolerance;
        ...
        %   See also generate_candidate_space, AnchorPoint, PlaneOrientationAnchor.
            ...
            if(obj.CheckRedundantTrimmingUpdate)
                [obj,hullPointMinMax] = obj.remove_redundant_planes();
            end
        ...
        %   See also generate_candidate_space, is_in_candidate_space.
            ...
            if(~isempty(obj.AnchorPoint))
                if(strcmpi(obj.GrowthDirectionType,'plane-direction'))
                    directionGrowth = -obj.PlaneOrientationAnchor;
                elseif(strcmpi(obj.GrowthDirectionType,'away-from-center'))
                    directionGrowth = (obj.AnchorPoint - center)./designSpaceNormalization;
                else
                    error('CandidateSpacePlanarTrimming:GrowthDirectionType',...
                        'Growth direction type %s is not supported.',obj.GrowthDirectionType);
                end
        ...
        function [isInside, score] = is_in_candidate_space(obj,designSample,varargin)
        ...
        %   See also is_in_convex_hull_with_plane.
            ...
            parser = inputParser;
            parser.addOptional('IncludeBoundingBox',true,@islogical);
            parser.addParameter('DistanceToPlaneTolerance',0,@isnumeric);
            ...
            % compute dot product between design samples and planes
            % A design is removed by a given plane if (anchor - design) dot plane > 0
            % or anchor dot plane - design dot plane > 0
            planeTerm = obj.PlaneOrientationAnchor ./ obj.NormalizationFactor; % (nAnchor,nDimension)
            dotProduct = (sum(obj.AnchorPoint.*planeTerm,2)' - (designSample*planeTerm')) - parser.Results.DistanceToPlaneTolerance; % (nSample,nAnchor)
            if(obj.ComputeVariantScore)
                [isInside,score] = design_deficit_to_label_score(dotProduct);
            else
                isInside = all(dotProduct<=0,2);
                score = max(dotProduct,[],2);
            end

            % check if it's inside the bounding box
            if(nargin<3 || parser.Results.IncludeBoundingBox)
        ...
        %   See also plot_convex_hull_2d, plot_convex_hull_3d.
            ...
            parser.addParameter('Tolerance',1e-9,@isnumeric);
            parser.parse(varargin{:});
            ...
            if(nDimension>3)
                warning('CandidateSpacePlanarTrimming:Plot:UnsupportedDimension',...
                    'Candidate space has %d dimensions, which is not supported for plotting. Consider fixing some design variables using the FixedVariables option.',nDimension);
                if(nargout>0)
                    plotHandle = [];
                end
                return;
            ...
            hullPoint = unique(obj.DesignSampleDefinition(obj.IsInsideDefinition,:),'rows');
            if(~isempty(fixedVariables))
                ...
                isInside = obj.is_in_candidate_space(projectedPoints,'DistanceToPlaneTolerance',parser.Results.Tolerance);
                if(any(isInside))
                ...
                for i=1:nAnchor
                    ...
                    isInside = obj.is_in_candidate_space(candidateHullPoint,'DistanceToPlaneTolerance',parser.Results.Tolerance);
                    if(any(isInside))
                        hullPoint = unique([hullPoint;candidateHullPoint(isInside,:)],'rows');
                    end
                end

                % project to each border of the design space as well
                for i=1:nDimension
                    % project to lower boundary
                    candidateHullPoint = obj.DesignSampleDefinition;
                    candidateHullPoint(:,i) = obj.DesignSpaceLowerBound(i);
                    ...
                    % project to upper boundary
                    candidateHullPoint = obj.DesignSampleDefinition;
                    candidateHullPoint(:,i) = obj.DesignSpaceUpperBound(i);
    ...
    methods (Access = private)
        function [obj,hullPointMinMax] = remove_redundant_planes(obj)
            hullPointMinMax = [];
            ...
            % Stage 1: remove redundant parallel planes (same orientation)
            planeTerm = obj.PlaneOrientationAnchor ./ obj.NormalizationFactor; % (nAnchor,nDimension)
            planeTermRounded = round(planeTerm,obj.RedundancyDigitRoundingTolerance);

            % gather planes with same(-ish) orientation and select the one with 
            % the most forward-facing threshold
            [~,~,planeOrientationGroupIndex] = unique(planeTermRounded,'rows');
            nGroup = max(planeOrientationGroupIndex);
            if(nGroup<nAnchor) % only check if there's any degree of redundancy
                anchorTerm = sum(obj.AnchorPoint.*planeTerm,2); % (nAnchor,1)
                isRedundantAnchor = false(nAnchor,1);
                for iGroup = 1:nGroup
                    iCurrentGroup = find(planeOrientationGroupIndex==iGroup);
                    if(numel(iCurrentGroup)<=1)
                        continue;
                    end
                    [~,iKeepLocal] = max(anchorTerm(iCurrentGroup));
                    iKeep = iCurrentGroup(iKeepLocal);
                    isRedundantAnchor(iCurrentGroup) = true;
                    isRedundantAnchor(iKeep) = false;
                end
            ...
            % Stage 2: project points to planes and check if any points are inside
            if(obj.PerformDetailedRedundancyCheck && nAnchor>1)
                ...
                nSample = size(pointsToMove,1);
                nDimension = size(pointsToMove,2);
                isRedundantAnchor = false(nAnchor,1);

                % batch to limit memory and reduce the number of calls to the 
                ...
                for iStart = 1:batchSizeMax:nAnchor
                    ...
                    % project points to planes
                    allCandidatePoints = nan(batchSize*nSample,nDimension);
                    for iBatchElement = 1:batchSize
                        iAnchor = iBatchAnchor(iBatchElement);
                        distanceToAnchor = (obj.AnchorPoint(iAnchor,:) - pointsToMove)./obj.NormalizationFactor(iAnchor,:);
                        dotProduct = sum(distanceToAnchor.*obj.PlaneOrientationAnchor(iAnchor,:),2);
                        distanceToPlane = obj.NormalizationFactor(iAnchor,:).*obj.PlaneOrientationAnchor(iAnchor,:).*dotProduct;
                        allCandidatePoints((iBatchElement-1)*nSample + (1:nSample),:) = pointsToMove + distanceToPlane;
                    end

                    % check if any points are inside the candidate space
                    isInsideAll = obj.is_in_candidate_space(allCandidatePoints,false);

                    % check if any points are inside the candidate space for each anchor    
                    for iBatchElement = 1:batchSize
                        iStartCurrentAnchor = (iBatchElement-1)*nSample + 1; 
                        iEndCurrentAnchor = iBatchElement*nSample;
                        if(~any(isInsideAll(iStartCurrentAnchor:iEndCurrentAnchor)))
                            isRedundantAnchor(iBatchAnchor(iBatchElement)) = true;
                        end
                    end

                    % find points inside that are shape-defining
                    insidePoints = [allCandidatePoints(isInsideAll,:);hullPointMinMax];
                    [~,iHullPointMin] = min(insidePoints,[],1);
                    [~,iHullPointMax] = max(insidePoints,[],1);
                    hullPointMinMax = insidePoints(unique([iHullPointMin,iHullPointMax]),:);
                end
