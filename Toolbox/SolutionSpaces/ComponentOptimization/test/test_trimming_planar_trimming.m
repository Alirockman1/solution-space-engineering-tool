...
iRemove = 1;
options = {'NormalizeVariables',true,'TrimmingSlack',0.5,'ConsiderOnlyKeepInSlack',true};

isKeep = [true(size(designSample,1),1);false(size(designPadding,1),1)];
isKeep(iRemove) = false;
[removalCandidate,removalInformation] = component_trimming_method_planar_trimming(designSampleComponent,iRemove,isKeep,options{:});
...
for i=1:size(removalCandidate,2)
    ...
    title(sprintf('Plane %d - Direction = [%d %d]',i,removalInformation(i).PlaneOrientationInside(1),removalInformation(i).PlaneOrientationInside(2)));
