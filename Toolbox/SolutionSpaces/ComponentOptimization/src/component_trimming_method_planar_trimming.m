...
%   limitations under the License.
    ...
    parser.addParameter('NormalizeVariables',true);
    parser.addParameter('Tolerance',0,@isscalar);
    ...
    % for each plane, points being removed are all whose dot product between
    % the distance to the anchor and each normal is non-positive. Avoid 3D
    % arrays; use 2D BLAS ops instead.
    nPlaneOrientation = size(normalizedPlaneOrientation,1);

    anchorVector = designSampleComponent(iRemove,:);
    distanceToAnchor = (anchorVector - designSampleComponent)./normalizationFactor; % (nSample,nDimension)
    dotProduct = distanceToAnchor * normalizedPlaneOrientation' - options.Tolerance; % (nSample,nPlaneOrientation)

    if(options.TrimmingSlack<1)
        isKeepInSlack = (dotProduct<=0); % (nSample,nPlaneOrientation)
        if(options.ConsiderOnlyKeepInSlack)
            isKeepInSlack = isKeepInSlack & isKeep;
        end

        dotProductForSlack = dotProduct;
        dotProductForSlack(~isKeepInSlack) = -inf;
        % column-wise minimum of -dotProduct over allowed rows
        slackPerPlane = min(-dotProductForSlack,[],1); % (1,nPlaneOrientation)
        % updating anchor along normal by (1-slack)*slackPerPlane adds this
        % scalar to each column of dotProduct (normals are unit-length)
        dotProduct = dotProduct + (1-options.TrimmingSlack).*slackPerPlane;

        % Per-plane anchor after slack adjustment
        anchorOffset = normalizedPlaneOrientation .* ((1-options.TrimmingSlack).*slackPerPlane(:)) .* normalizationFactor; % (nPlaneOrientation,nDimension)
        anchorPerPlane = anchorVector + anchorOffset;
    else
        anchorPerPlane = repmat(anchorVector,nPlaneOrientation,1); % (nPlaneOrientation,nDimension)
    end

    removalCandidate = (dotProduct>0);
    if(nargout>1)
        for i=1:nPlaneOrientation
            removalInformation(i).Anchor = anchorPerPlane(i,:);
