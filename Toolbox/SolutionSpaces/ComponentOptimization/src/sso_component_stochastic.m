...
%   limitations under the License.
    ...
    while((~convergedExploration) && (iExploration<=options.MaxIterExploration))
        ...
        if(options.UseAdaptiveGrowthRate && iExploration>1)
            ...
            if(purity<1 || ~options.DoNotAdaptGrowthRateOnFullPurity)
        ...
        % truncate padding samples if there are too many
        if(~isempty(options.MaximumNumberPaddingSamples) && nPaddingGenerated>options.MaximumNumberPaddingSamples)
            ...
            nPaddingGenerated = options.MaximumNumberPaddingSamples;
        end
        % generate more padding samples if there are too few
        if(~isempty(options.MinimumNumberPaddingSamples) && size(paddingSample,1)<options.MinimumNumberPaddingSamples)
            ...
            nPaddingGenerated = nPaddingGenerated + nPaddingMissing;
        end
        ...
        if(nAcceptable==0 || nUseful==0 || nAcceptableUseful==0 || (options.UseAdaptiveGrowthRate && purity<options.MinimumPurityReset))
            ...
            if(isOutputOptimizationData)
                ...
                optimizationData.IterationData(iLog) = struct(... 
                    ...
                    'NumberGoodDesigns',sum(isGoodPerformance),...
                    'NumberPhysicallyFeasibleDesigns',sum(isPhysicallyFeasible),...
                    'NumberAcceptableAndUsefulDesigns',nAcceptableUseful,...
                    'NumberAcceptableDesigns',nAcceptable,...
                    'NumberUsefulDesigns',nUseful,...
                    'ComponentMeasureBeforeTrim',componentMeasureGrown,...
                    'ComponentMeasureAfterTrim',componentMeasureTrimmed,...
                    ... % system data
                    'EvaluatedDesignSamples',ternary(options.SaveIterationAllData,designSample,[]),...
                    'EvaluationOutput',ternary(options.SaveIterationAllData,outputEvaluation,[]),...
                    ... % problem data
                    'DesignScore',ternary(options.SaveIterationAllData,score,[]),...
                    'IsGoodPerformance',ternary(options.SaveIterationAllData,isGoodPerformance,[]),...
                    'IsPhysicallyFeasible',ternary(options.SaveIterationAllData,isPhysicallyFeasible,[]),...
                    'IsAcceptable',ternary(options.SaveIterationAllData,isAcceptable,[]),...
                    'IsUseful',ternary(options.SaveIterationAllData,isUseful,[]),...
                    'PaddingSamplesUsed',ternary(options.SaveIterationAllData,paddingSample,[]),...
                    ... % trimming data
                    'SamplingBoxBeforeTrim',ternary(options.SaveIterationAllData,samplingBoxGrown,[]),...
                    'SamplingBoxAfterTrim',ternary(options.SaveIterationAllData,samplingBox,[]),... 
                    'CandidateSpacesBeforeTrim',ternary(options.SaveIterationAllData,candidateSpaceGrown,[]),...
                    'CandidateSpacesAfterTrim',ternary(options.SaveIterationAllData,candidateSpace,[]),...
                    ... % timing data
        ...
        if(candidateSpaceDefined && options.UseShapeSamplesExploration)
            % use not grown sampling box to increase chance of generated points being inside the candidate space
            [shapeSample,timeElapsedShape] = sso_component_sub_find_shape_defining_design_points(options.ShapeTypeExploration,candidateSpaceGrown,componentIndex,samplingBox,options.CandidateSpaceSamplingFunction,candidateSpaceSamplingOptions,console);
        end
        ...
        if(isOutputOptimizationData)
            ...
            optimizationData.IterationData(iLog) = struct(... 
                ...
                ... % trimming data
                'SamplingBoxBeforeTrim',ternary(options.SaveIterationAllData,samplingBoxGrown,[]),...
                'SamplingBoxAfterTrim',ternary(options.SaveIterationAllData,samplingBoxTrimmed,[]),... 
                'CandidateSpacesBeforeTrim',ternary(options.SaveIterationAllData,candidateSpaceGrown,[]),...
                'CandidateSpacesAfterTrim',ternary(options.SaveIterationAllData,candidateSpaceTrimmed,[]),...
                ... % timing data
                'TimeElapsedAdaptGrowthRate',timeElapsedAdaptGrowth,...
                'TimeElapsedGrow',timeElapsedGrow,...
                'TimeElapsedGenerate',timeElapsedGenerate,...
    ...
    while((~convergedConsolidation) && (iConsolidation<=options.MaxIterConsolidation))
        ...
        if(performTrim)
            % get shape definition for each candidate space
            [shapeSample,timeElapsedShape] = sso_component_sub_find_shape_defining_design_points(options.ShapeTypeConsolidation,candidateSpace,componentIndex,samplingBox,options.CandidateSpaceSamplingFunction,candidateSpaceSamplingOptions,console);
        ...
        if(isOutputOptimizationData)
            ...
            optimizationData.IterationData(iLog) = struct(... 
                ...
                'PaddingSamplesUsed',ternary(options.SaveIterationAllData,paddingSample,[]),...
                ... % trimming data
                'SamplingBoxBeforeTrim',ternary(options.SaveIterationAllData,samplingBox,[]),...
                'SamplingBoxAfterTrim',ternary(options.SaveIterationAllData,samplingBoxTrimmed,[]),... 
                'CandidateSpacesBeforeTrim',ternary(options.SaveIterationAllData,candidateSpace,[]),...
                'CandidateSpacesAfterTrim',ternary(options.SaveIterationAllData,candidateSpaceTrimmed,[]),...
                ... % timing data
...
%%
function [shapeSample,timeElapsedShape] = sso_component_sub_find_shape_defining_design_points(shapeType,candidateSpace,componentIndex,samplingBox,candidateSpaceSamplingFunction,candidateSpaceSamplingOptions,console)
    console.info('Finding shape-defining design points... ');
    timeStartShape = tic;

    if(strcmpi(shapeType,'advanced'))
        shapeSample = sso_component_sub_find_shape_defining_design_points_advanced(candidateSpace,componentIndex,candidateSpaceSamplingFunction,candidateSpaceSamplingOptions);
    elseif(strcmpi(shapeType,'simple'))
        options = parser_variable_input_to_structure(candidateSpaceSamplingOptions{:});
        samplingMethodFunction = options.SamplingMethodFunction;
        samplingMethodOptions = options.SamplingMethodOptions;
        shapeSample = sso_component_sub_find_shape_defining_design_points_simple(candidateSpace,componentIndex,samplingBox,samplingMethodFunction,samplingMethodOptions);
    else
        error('Invalid shape type: %s',shapeType);
    end
...
function shapeSample = sso_component_sub_find_shape_defining_design_points_advanced(candidateSpace,componentIndex,candidateSpaceSamplingFunction,candidateSpaceSamplingOptions)
    nComponent = length(candidateSpace);
    for i=1:nComponent
        designSampleDefinition = candidateSpace(i).DesignSampleDefinition;
        isShapeDefinition = candidateSpace(i).IsShapeDefinition;
        shapeDefinition{i} = designSampleDefinition(isShapeDefinition,:);
    end

    nShape = cellfun(@(x)size(x,1),shapeDefinition);
    nDimension = sum(cellfun(@(x)size(x,2),shapeDefinition));
    shapeSample = nan(sum(nShape),nDimension);

    rowIndex = 1;
    for i=1:nComponent
        % generate complementary samples
        otherIndex = setdiff(1:nComponent,i);
        complementarySample = candidateSpaceSamplingFunction(candidateSpace(otherIndex),componentIndex(otherIndex),nShape(i),candidateSpaceSamplingOptions{:});

        shapeSample(rowIndex:rowIndex+nShape(i)-1,componentIndex{i}) = shapeDefinition{i};
        for j=1:length(otherIndex)
            shapeSample(rowIndex:rowIndex+nShape(i)-1,componentIndex{otherIndex(j)}) = complementarySample(:,componentIndex{otherIndex(j)});
        end
        rowIndex = rowIndex + nShape(i);
    end
end

function shapeSample = sso_component_sub_find_shape_defining_design_points_simple(candidateSpace,componentIndex,samplingBox,samplingMethodFunction,samplingMethodOptions)
    nComponent = length(candidateSpace);
    for i=1:nComponent
        designSampleDefinition = candidateSpace(i).DesignSampleDefinition;
        isShapeDefinition = candidateSpace(i).IsShapeDefinition;
        isInsideDefinition = candidateSpace(i).IsInsideDefinition;

        shapeDefinitionInside{i} = designSampleDefinition(isShapeDefinition & isInsideDefinition,:);
        shapeDefinitionOutside{i} = designSampleDefinition(isShapeDefinition & ~isInsideDefinition,:);
        % shapeDefinition{i} = designSampleDefinition(isShapeDefinition,:);
    end

    shapeSampleInside = sso_component_sub_make_shape_sample_simple(shapeDefinitionInside,samplingBox,componentIndex,samplingMethodFunction,samplingMethodOptions);
    shapeSampleOutside = sso_component_sub_make_shape_sample_simple(shapeDefinitionOutside,samplingBox,componentIndex,samplingMethodFunction,samplingMethodOptions);
    shapeSample = [shapeSampleInside;shapeSampleOutside];
    % shapeSample = sso_component_sub_make_shape_sample_simple(shapeDefinition,samplingBox,componentIndex,samplingMethodFunction,samplingMethodOptions);
end

function shapeSample = sso_component_sub_make_shape_sample_simple(shapeDefinition,samplingBox,componentIndex,samplingMethodFunction,samplingMethodOptions)
    nShape = cellfun(@(x)size(x,1),shapeDefinition);
    maxShape = max(nShape);

    nDimension = size(samplingBox,2);
    ...
    for i=1:nComponent
        ...
        if(nShapeMissing>0)
            shapeMissing = samplingMethodFunction(samplingBox(:,componentIndex{i}),nShapeMissing,samplingMethodOptions{:});
