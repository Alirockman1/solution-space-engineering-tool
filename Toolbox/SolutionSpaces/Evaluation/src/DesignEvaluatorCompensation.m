...
%   limitations under the License.
    ...
    properties(SetAccess = protected)
        ...
        %   USECLOSESTEVALUATEDASINITIALDESIGN determines whether the closest evaluated
        %   design is used as the initial design for the next optimization.
        %   This is done to reduce the number of optimization iterations necessary.
        %
        %   USECLOSESTEVALUATEDASINITIALDESIGN : logical
        %   
        %   See also design_sort_min_distance, design_optimize_performance_score.
        UseClosestEvaluatedAsInitialDesign

        %DO NOTOPTIMIZEIFINITIALISFEASIBLE determines whether the optimization is not done if the initial design is feasible.
        %   DO NOTOPTIMIZEIFINITIALISFEASIBLE determines whether the optimization is not done if the initial design is feasible.
        %   This is done to reduce the number of optimization iterations necessary.
        %   If the initial design is feasible, the optimization is not done and the current design is returned.
        %
        %   DO NOTOPTIMIZEIFINITIALISFEASIBLE : logical
        %   
        %   See also design_optimize_performance_score.
        DoNotOptimizeIfInitialIsFeasible
    end
    ...
    methods
        ...
        %   design_sort_min_distance.
            ...
            parser.addParameter('UseClosestEvaluatedAsInitialDesign',true,@islogical);
            parser.addParameter('DoNotOptimizeIfInitialIsFeasible',true,@islogical);
            ...
            obj.UseClosestEvaluatedAsInitialDesign = options.UseClosestEvaluatedAsInitialDesign;
            obj.DoNotOptimizeIfInitialIsFeasible = options.DoNotOptimizeIfInitialIsFeasible;
        end
        ...
        %   See also design_sort_min_distance, design_optimize_performance_score.
            ...
            if(isFullSpaceSample)
                if(obj.AcceptFullSpaceSample)
                    ...
                    bspaceInitial = designSample(:,~obj.CompensationAspaceIndex);
            ...
            elseif(size(designSample,2)==nDesignVariableAspace)
                ...
                if(size(bspaceInitial,1)==1)
                    bspaceInitial = repmat(bspaceInitial,nSample,1);
                end
            else
            ...
            optimizedAspaceSample = nan(0,nDesignVariableAspace);
            optimizedBspaceSample = nan(0,nDesignVariableBspace);
            ...
            designSample = designSample(minDistanceOrder,:);
            bspaceInitial = bspaceInitial(minDistanceOrder,:);

            % find the optimal B-space values for each design
            iNext = 1;
            aspaceSample = designSample(iNext,:);
            bspaceSample = bspaceInitial(iNext,:);

            isDoneWithAllPoints = false;
            while(~isDoneWithAllPoints)
                currentDesign = nan(1,nDesignVariable);
                currentDesign(obj.CompensationAspaceIndex) = aspaceSample;
                currentDesign(~obj.CompensationAspaceIndex) = bspaceSample;

                [performanceDeficitCurrent,physicalFeasibilityDeficitCurrent,evaluatorOutputCurrent] = ...
                    obj.BaseDesignEvaluator.evaluate(currentDesign);
                [~,scoreCurrent] = design_deficit_to_label_score(performanceDeficitCurrent);
                isPhysicallyFeasible = isempty(physicalFeasibilityDeficitCurrent) || all(physicalFeasibilityDeficitCurrent<=0);

                % allocate output if not already done
                if(isempty(performanceDeficit))
                    performanceDeficit = nan(nSample,size(performanceDeficitCurrent,2));
                end
                % still "empty" even when already allocated if no physical feasibility deficit, use second check to avoid allocating again
                if(isempty(physicalFeasibilityDeficit) && size(physicalFeasibilityDeficit,1)==0)
                    physicalFeasibilityDeficit = nan(nSample,size(physicalFeasibilityDeficitCurrent,2));
                end

                if(scoreCurrent<=0 && isPhysicallyFeasible && obj.DoNotOptimizeIfInitialIsFeasible)
                    optimizationOutputCurrent = [];
                else
                    [currentDesign,scoreCurrent,optimizationOutputCurrent] = ...
                    ...
                    bspaceSample = currentDesign(~obj.CompensationAspaceIndex);

                    optimizedAspaceSample = [optimizedAspaceSample;aspaceSample];
                    optimizedBspaceSample = [optimizedBspaceSample;bspaceSample];
                    [performanceDeficitCurrent,physicalFeasibilityDeficitCurrent,evaluatorOutputCurrent] = ...
                        obj.BaseDesignEvaluator.evaluate(currentDesign);
                    
                end
                performanceDeficit(iNext,:) = performanceDeficitCurrent;
                physicalFeasibilityDeficit(iNext,:) = physicalFeasibilityDeficitCurrent;
                optimizationOutput{iNext} = optimizationOutputCurrent;
                evaluatorOutput{iNext} = evaluatorOutputCurrent;
                bspaceOptimal(iNext,:) = bspaceSample;
                scoreOptimal(iNext) = scoreCurrent;
                
                % find next design to evaluate
                hasBeenEvaluated = ~isnan(scoreOptimal);
                if(all(hasBeenEvaluated))
                    isDoneWithAllPoints = true;
                else
                    iNext = iNext + 1;
                    aspaceSample = designSample(iNext,:);

                    if(~isempty(optimizedAspaceSample) && obj.UseClosestEvaluatedAsInitialDesign)
                        iClosestEvaluated = knnsearch(optimizedAspaceSample,aspaceSample,obj.AspaceOrderKnnsearchOptions{:});
                        bspaceSample = optimizedBspaceSample(iClosestEvaluated,:);
                    else
                        bspaceSample = bspaceInitial(iNext,:);
                    end
                end
            ...
            % re-order
            performanceDeficit(minDistanceOrder,:) = performanceDeficit;
            physicalFeasibilityDeficit(minDistanceOrder,:) = physicalFeasibilityDeficit;
            optimizationOutput(minDistanceOrder) = optimizationOutput;
            evaluatorOutput(minDistanceOrder) = evaluatorOutput;
            bspaceOptimal(minDistanceOrder,:) = bspaceOptimal;
            scoreOptimal(minDistanceOrder) = scoreOptimal;
