function arrayRgb = convert_base10hexcode_to_rgb(hex10Rgb,nDigit)
...
%   limitations under the License.
    ...
    redColor = round(hex10Rgb - quotientAfterRed.*baseDivider);
    
    % G
    quotientAfterGreen = floor(quotientAfterRed./baseDivider);
    greenColor = round(quotientAfterRed - quotientAfterGreen.*baseDivider);
    
    % B
    quotientAfterBlue = floor(quotientAfterGreen./baseDivider); % should be 0
    blueColor = round(quotientAfterGreen - quotientAfterBlue.*baseDivider);

    % wrap
    arrayRgb = [redColor(:),greenColor(:),blueColor(:)]/(baseDivider-1);
end